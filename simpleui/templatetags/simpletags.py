...
@register.simple_tag(takes_context=True)
def menus_old(context, _get_config=None):
    data = []
...
def createMenu(lisMenu):
    ...
    firstMenu = [x for x in lisMenu if x['parent_id'] is None]

    def innerMenu(parentId, parentMenu):
        chilMenu = [x for x in lisMenu if x['parent_id'] == parentId]

        for m in chilMenu:

            if m['res_type'] == 'M' and (m['res_value'] is None or str(m['res_value']).strip() == ''):
                menu = {}
                menu['name'] = m['res_name']
                menu['icon'] = ''
                menu['models'] = []
                parentMenu['models'].append(menu)

                # 递归调用
                innerMenu(m['id'], menu)
            elif m['res_type'] == 'M' and m['res_value'] is not None and str(m['res_value']).strip() != '':
                menu = {}
                menu['name'] = m['res_name']
                menu['icon'] = ''
                menu['url'] = m['res_value']
                menu['actives'] = []
                parentMenu['models'].append(menu)
                ...
                parentMenu['actives'].append(m['res_value'])

    for m0 in firstMenu:
        menu0 = {}
        menu0['name'] = m0['res_name']
        menu0['icon'] = ''
        menu0['models'] = []
        innerMenu(m0['id'], menu0)
        mainMenu.append(menu0)
    return mainMenu
...
def menus(context, _get_config=None):
    ...
    # --------------------添加自定义菜单集合 anton begin----------------------------------
    mainMenu = []
    lisMenu = context.request.session.get("permissions_list")
    if lisMenu and len(lisMenu) > 0:
        mainMenu = createMenu(lisMenu)
